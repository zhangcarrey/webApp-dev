<?php
	...
	// 
	$handleResult = file_get_contents($handleUrl); //'{}'

	// $recResult = file_get_contents($recUrl); //'{}'

	// 转成数组
	$handleResult = json_decode($handleResult, true);
	// 转成数组
	// $recResult = json_decode($recResult, true);

	// var_dump($handleResult);

	// 拼凑数据
	// $result = array('handle'=>$handleResult, 'rec'=>$recResult);

	// 返回json数据
	echo json_encode($handleResult);
