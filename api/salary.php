<?php
	...
	// 
	$salaryResult = file_get_contents($salaryUrl); //'{}'

	// $recResult = file_get_contents($recUrl); //'{}'

	// 转成数组
	$salaryResult = json_decode($salaryResult, true);
	// 转成数组
	// $recResult = json_decode($recResult, true);

	// var_dump($salaryResult);

	// 拼凑数据
	// $result = array('salary'=>$salaryResult, 'rec'=>$recResult);

	// 返回json数据
	echo json_encode($salaryResult);
