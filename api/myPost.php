<?php
	...
	// 
	$myPostResult = file_get_contents($myPostUrl); //'{}'

	// $recResult = file_get_contents($recUrl); //'{}'

	// 转成数组
	$myPostResult = json_decode($myPostResult, true);
	// 转成数组
	// $recResult = json_decode($recResult, true);

	// var_dump($myPostResult);

	// 拼凑数据
	// $result = array('myPost'=>$myPostResult, 'rec'=>$recResult);

	// 返回json数据
	echo json_encode($myPostResult);
