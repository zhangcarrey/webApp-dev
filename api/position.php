<?php
	...
	// 全部
	$positionUrl = '../data/position.json';
	// 推荐
	...
	// 
	$positionResult = file_get_contents($positionUrl); //'{}'

	// $recResult = file_get_contents($recUrl); //'{}'

	// 转成数组
	$positionResult = json_decode($positionResult, true);
	// 转成数组
	// $recResult = json_decode($recResult, true);

	// var_dump($positionResult);

	// 拼凑数据
	// $result = array('position'=>$positionResult, 'rec'=>$recResult);

	// 返回json数据
	echo json_encode($positionResult);
