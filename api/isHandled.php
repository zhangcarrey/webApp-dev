<?php
	...
	// 全部
	$isHandledUrl = '../data/isHandled.json';
	// 推荐
	...
	// 
	$isHandledResult = file_get_contents($isHandledUrl); //'{}'

	// $recResult = file_get_contents($recUrl); //'{}'

	// 转成数组
	$isHandledResult = json_decode($isHandledResult, true);
	// 转成数组
	// $recResult = json_decode($recResult, true);

	// var_dump($isHandledResult);

	// 拼凑数据
	// $result = array('isHandled'=>$isHandledResult, 'rec'=>$recResult);

	// 返回json数据
	echo json_encode($isHandledResult);
