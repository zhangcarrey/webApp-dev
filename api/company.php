<?php
	...
	// 
	$companyResult = file_get_contents($companyUrl); //'{}'

	// $recResult = file_get_contents($recUrl); //'{}'

	// 转成数组
	$companyResult = json_decode($companyResult, true);
	// 转成数组
	// $recResult = json_decode($recResult, true);

	// var_dump($companyResult);

	// 拼凑数据
	// $result = array('company'=>$companyResult, 'rec'=>$recResult);
