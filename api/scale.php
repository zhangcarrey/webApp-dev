<?php
	...
	// 
	$scaleResult = file_get_contents($scaleUrl); //'{}'

	// $recResult = file_get_contents($recUrl); //'{}'

	// 转成数组
	$scaleResult = json_decode($scaleResult, true);
	// 转成数组
	// $recResult = json_decode($recResult, true);

	// var_dump($scaleResult);

	// 拼凑数据
	// $result = array('scale'=>$scaleResult, 'rec'=>$recResult);

	// 返回json数据
	echo json_encode($scaleResult);
